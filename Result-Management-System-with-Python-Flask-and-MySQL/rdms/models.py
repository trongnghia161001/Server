...
class Files(db.Model):
    ...
    user = db.relationship("Users", foreign_keys=[id_user], backref="files")
    def __repr__(self):
        return f"File: {self.filename} in Bucket: {self.bucket}"
...
class Errors(db.Model):
    ...
    related_file = db.relationship("Files", foreign_keys=[id_file], backref="errors")
    def __repr__(self):
        return f"Error in File: {self.filename} in Bucket: {self.bucket}"
...
class ProfileView(ModelView):
    ...
    def validate_form(self, form):
        ...
        if (request.method == "POST" and 
            ...
            flash(f'KẾT QUẢ already exists for {form_email}', 'error')
            return False
